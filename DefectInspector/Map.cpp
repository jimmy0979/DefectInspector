...
Mat Map::paint_square(const int& x, const int& y)
{
	...
	rectangle(this->img, Rect(x * (map_W / divide), y * (map_L / divide), map_W / divide, map_L / divide), Scalar(255, 255, 255));//繪製被選擇區域的方框
	return this->img;
}
...
{
	Map::img = cv::Scalar(255,255,255);//繪製底色
	...
	rectangle(Map::img, Rect(x * (map_W / divide), y * (map_L / divide), map_W / divide, map_L / divide), Scalar(255, 255, 255));//paint selected square
	return Map::img;
}
